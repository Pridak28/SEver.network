...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // CRITICAL FIX: Force loading overlay removal after 4 seconds (GUARANTEED)
  setTimeout(() => {
  ...
  // Only show the Connect button in the navbar on mobile
  const navbarConnect = document.querySelector(".navbar-connect");
  if (navbarConnect) {
    if (window.innerWidth >= 768) {
      navbarConnect.style.display = "none";
    } else {
      navbarConnect.style.display = "";
    }
  ...
  // === Remove mobile overlay hiding - let loading animation run normally ===
  // Removed mobile-specific code that was immediately hiding the loading overlay
  // This allows the loading animation to run for the full 4 seconds on all devices

  // Log JS errors to a visible alert on mobile
  // Removed mobile JS error alert to prevent popups on main website
 ...
 */
function updateDateTimeElements(loadingOverlay) {
...
function removeLoadingOverlay(loadingOverlay) {
  ...
  setTimeout(completeLoadingOverlayRemoval, 600, loadingOverlay);

  // Start background animation after loading completes
  setTimeout(() => {
    if (window.bgAnimation) {
      window.bgAnimation.show();
    }
  }, 700);
}
...
function setupLoadingSequence(loadingOverlay, loadingStatus) {
  ...
  try {
    ...
    // Set timeout to exactly 4000ms (4 seconds) as requested
    setTimeout(() => removeLoadingOverlay(loadingOverlay), 4000);

    // Backup timeout with a slightly longer delay for safety
    setTimeout(function () {
...
function initializeLoadingAnimation() {
  ...
  console.log('✅ Loading overlay found, setting 4 second timer...');

  // FAILSAFE: Always remove loading overlay after maximum 4 seconds
  setTimeout(() => {
    console.log('⏰ 4 seconds elapsed, removing loading overlay...');
    if (document.getElementById("loadingOverlay")) {
      removeLoadingOverlay(document.getElementById("loadingOverlay"));
    }
  }, 4000);

  // Ensure the loading overlay is visible and styled correctly
  ...
  try {
    ...
    // If any error, remove immediately
    removeLoadingOverlay(loadingOverlay);
  }

  // Prevent background animation from starting until loading is done
  window.backgroundAnimationBlocked = true;

  // Unblock background animation after loading completes
  setTimeout(() => {
    window.backgroundAnimationBlocked = false;
  }, 4500);
}
...
function initializeBackgroundAnimation() {
  console.log("Initializing background animation system...");

  // DISABLED - Let simple-bg-animation.js handle everything
  // The fallback animation conflicts with our main animation
  console.log("Background animation handled by simple-bg-animation.js");
  return;

  // Check if simple background animation is available
  if (typeof window.simpleBgAnimation !== "undefined") {
    // Refresh the background animation to ensure it's working
    window.simpleBgAnimation.refresh();
    console.log("Background animation system activated");
  } else {
    // Simple fallback animation if the main system isn't loaded
    createFallbackBackgroundAnimation();
  }
}

/**
 * Fallback background animation if main system fails
 */
function createFallbackBackgroundAnimation() {
  console.log("Creating fallback background animation...");

  // Remove any existing background canvases to avoid conflicts
  document
    .querySelectorAll("#fallback-bg-canvas, #bgCanvas")
    .forEach((canvas) => {
  ...
  // Create a simple canvas-based animation
  const canvas = document.createElement("canvas");
  canvas.id = "fallback-bg-canvas";
  ...
  function animate() {
    ...
    particles.forEach((particle, index) => {
      ...
      const pulseAlpha =
        particle.alpha + Math.sin(time * particle.pulseSpeed + index) * 0.3;
  ...
  console.log(
    "Fallback background animation created with",
    particleCount,
    "particles"
  );

  // Store cleanup function
  window.cleanupFallbackAnimation = function () {
    if (animationId) {
...
function initMobileNavigation() {
  ...
  if (!hamburgerMenu || !navMenu || hamburgerMenu.dataset.initialized === "true") {
    return;
  }
  ...
  hamburgerMenu.setAttribute("aria-controls", "navMenu");
  ...
  navMenu.addEventListener("focusout", function (event) {
    if (
      ...
      !navMenu.contains(event.relatedTarget)
    ) {
      closeMenu();
    }
  });
...
function initializeCore() {
  ...
  // Ensure background animation is initialized
...
/* Theme toggle & mobile nav handled during initialization */
...
function getThemeColor(variableName, opacity = 1) {
  ...
  const rgbString = getComputedStyle(root).getPropertyValue(variableName).trim();
  if (rgbString) {
    return `rgba(${rgbString}, ${opacity})`;
  }
  // Fallback to cyan if variable not found
  return `rgba(0, 255, 255, ${opacity})`;
}

// Declare chart variables in outer scope so they're accessible everywhere
let energyProductionChart = null;
let networkActivityChart = null;

// Initialize Energy Dashboard charts when DOM is ready
document.addEventListener('DOMContentLoaded', function() {
  console.log('🔄 Initializing Energy Dashboard charts...');

  // Prevent multiple initializations
  if (window.energyDashboardChartsInitialized) {
    console.warn('⚠️ Energy Dashboard charts already initialized, skipping...');
    return;
  }
  ...
  const energyProdCanvas = document.getElementById("energyProductionChart");

  if (!energyProdCanvas) {
    console.error('❌ energyProductionChart canvas not found in DOM');
  } else {
    console.log('✅ energyProductionChart canvas found');

    try {
      // Destroy existing chart if it exists
      const existingChart = Chart.getChart(energyProdCanvas);
      if (existingChart) {
        console.log('🗑️ Destroying existing energyProductionChart');
        existingChart.destroy();
      }

      const energyProductionCtx = energyProdCanvas.getContext("2d");
      console.log('✅ Got 2D context for energyProductionChart');

      energyProductionChart = new Chart(energyProductionCtx, {
    type: "line",
    data: {
      labels: [
        "Jan",
      ...
      datasets: [
        {
          ...
          borderColor: getThemeColor('--accent-color-rgb', 1),
          backgroundColor: getThemeColor('--accent-color-rgb', 0.2),
          borderWidth: 2,
          tension: 0.4,
        },
        {
    ...
    options: {
      ...
      console.log('✅ energyProductionChart created successfully!', energyProductionChart);
    } catch (error) {
      console.error('❌ Error creating energyProductionChart:', error);
    }
  }

  // Network Activity Chart
  const networkActivityCanvas = document.getElementById("networkActivityChart");

  if (!networkActivityCanvas) {
    console.error('❌ networkActivityChart canvas not found in DOM');
  } else {
    try {
      // Destroy existing chart if it exists
      const existingChart = Chart.getChart(networkActivityCanvas);
      if (existingChart) {
        console.log('🗑️ Destroying existing networkActivityChart');
        existingChart.destroy();
      }

      const networkActivityCtx = networkActivityCanvas.getContext("2d");
      networkActivityChart = new Chart(networkActivityCtx, {
    type: "bar",
    data: {
      labels: [
        "00:00",
    ...
    options: {
      ...
      console.error('❌ Error creating networkActivityChart:', error);
    }
  }
  ...
  // Mark as initialized
  window.energyDashboardChartsInitialized = true;
  console.log('✅ Energy Dashboard charts initialized successfully!');
}); // End of DOMContentLoaded for Energy Dashboard charts
...
function processNodeFiltering(node, filter) {
  ...
  const categoryAttr = node.getAttribute("data-categories") || "";
...
// Always attach button event listeners regardless of device or screen size
function attachButtonListeners() {
  ...
  // Add more button listeners here as needed
}
...
// Enhanced debugging and delayed initialization for buttons
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  setTimeout(() => {
    ...
    // Ensure buttons are initialized on mobile load
    if (window.innerWidth < 768) {
      initializeButtons();
    }
...
// --- MOBILE-ONLY BUTTON/LINK INTERACTION FIX ---
...
function mobileInteractionFix() {
  if (!isMobileDevice()) return;
  // Hide overlays/blockers, but DO NOT hide #loadingOverlay on mobile
  document.querySelectorAll(".overlay, .modal, .blocker").forEach((el) => {
...
// Observe DOM changes and re-apply the fix for dynamic content (mobile only)
function observeMobileDomChanges() {
  if (!isMobileDevice()) return;
...
// Run mobile fix on DOMContentLoaded, resize, and after DOM mutations
function setupMobileButtonFix() {
  if (!isMobileDevice()) return;
  document.addEventListener("DOMContentLoaded", mobileInteractionFix);
  window.addEventListener("resize", mobileInteractionFix);
  observeMobileDomChanges();
  // Also run after a short delay to catch late-loaded content
  setTimeout(mobileInteractionFix, 800);
  setTimeout(mobileInteractionFix, 2000);
}

setupMobileButtonFix();

// --- UNIVERSAL POINTERUP+CLICK HANDLER FOR ALL BUTTONS/LINKS ---
function addPointerUniversalHandlerAll() {
  document.querySelectorAll("button, a").forEach((btn) => {
    // Get all existing click handlers
    ...
    function once(e) {
      ...
      // Also dispatch a click event if not already
      if (e.type === "pointerup") {
    ...
    btn.addEventListener("pointerup", once);
  });
}
document.addEventListener("DOMContentLoaded", addPointerUniversalHandlerAll);
